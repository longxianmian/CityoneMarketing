...
export default function OpenInLinePage() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(180deg, #f2fbf8 0%, #ffffff 100%)', padding: 20 }}>
      <Card style={{ maxWidth: 420, width: '100%', borderRadius: 24, overflow: 'hidden', boxShadow: '0 20px 50px rgba(15, 111, 86, 0.12)' }} bodyStyle={{ padding: 0 }}>
        <div style={{ background: 'linear-gradient(135deg, #13c267 0%, #12b981 100%)', color: '#fff', padding: '30px 24px 28px', textAlign: 'center' }}>
          ...
          <div style={{ marginTop: 8 }}>
            <Tag color="rgba(255,255,255,0.18)" style={{ color: '#fff', border: 'none', paddingInline: 12, lineHeight: '24px', borderRadius: 999 }}>
              官方认证账号
            </Tag>
          </div>
        </div>
        <div style={{ padding: 24 }}>
          ...
          <Button
            ...
            style={{ height: 48, borderRadius: 999, fontWeight: 700 }}
          >
            关注 LINE OA 并继续
          </Button>
          <Button style={{ marginTop: 12, height: 44, borderRadius: 999 }} block onClick={() => navigate(returnPath, { replace: true })}>
            返回当前详情页
          </Button>
          <Button style={{ marginTop: 12, height: 44, borderRadius: 999 }} block onClick={handleResetIdentity}>
            重新识别 LINE 身份
          </Button>
        </div>
      </Card>
